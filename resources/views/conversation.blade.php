...
<div class="relative sm:flex min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
    ...
    <div style="width:90%; z-index:1; right:1">
        ...
        <h1 style="margin:20px; height:90%;" class="sub text-xl dark:text-white">  @if(isset($req)) {{ $req[0]['response']}}  @endif </h3>
        <form method="post" action=""> 
            ...
            @php
                ...
                $_SESSION['rand']=$token;
            @endphp
            <input type="hidden" name="hidden" id="hidden" value="{{$token}}">
            <input style="bottom: 2px; position:fixed; width:500px; height:50px; margin:10px;" type="text" name="conv" id="conv">
            <input style="bottom: 2px; position:fixed; margin:10px; height:50px; left:47%" class="dark:bg-red-800/20 dark:text-white btn" type="submit" name="submit" id="submit" value="send">
        </form>
    </div>
</div>
